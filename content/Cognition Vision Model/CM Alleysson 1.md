...
La rétine n'est pas un capteur uniforme. La répartition des cônes (L, M, S) diffère selon les individus, formant une mosaïque unique. On peut imager cette mosaïque in-vivo avec une optique adaptative (précision de 0.5 min d'arc).
...
- **Chrominance :** À cause de la périodicité de la grille de Bayer (alternance des pixels), l'information de couleur est modulée et repoussée vers les hautes fréquences (les bords du spectre, aux fréquences de Nyquist).
...
- Si l'espace des couleurs était euclidien (comme une feuille de papier millimétré), les seuils de tolérance seraient des cercles de même rayon partout.
...
### Vers une géométrie hyperbolique

Des études plus récentes suggèrent que la géométrie de l'espace des couleurs pourrait être hyperbolique (courbure négative), plutôt que simplement Riemannienne classique.
...
En vision par ordinateur 3D :

- Un objet a des coordonnées réelles $(X, Y, Z)$.
    
- Sur l'image (2D), tous les points alignés avec l'origine (l'œil) se projettent au même endroit. La profondeur est perdue.
...
1. Axe Temporel / Échelle (Rayon) $\leftrightarrow$ Voie Magno (Luminance) :
    ...
    Correspond à l'intensité ou la distance à l'origine dans l'espace projectif. La voie Magno code "combien" de lumière il y a, indépendamment de la teinte.
    
2. Plan Chromatique $\leftrightarrow$ Voie Parvo (Rouge/Vert) :
    
    Correspond aux coordonnées spatiales sur la surface de projection (l'hyperboloïde). C'est l'information de teinte fine.
    
3. Origine / Axe vertical $\leftrightarrow$ Voie Konio (Bleu/Jaune) :
    
    Lié à l'axe d'alignement du système (Plan pourpre).
    

### Pourquoi une hyperboloïde ?

Le modèle de Naka-Rushton (qui décrit la réponse saturante d'un neurone : $R = \frac{I}{I + \sigma}$) est mathématiquement équivalent à une projection sur une métrique hyperbolique (Disque de Poincaré)26.

Cela explique pourquoi l'espace des couleurs est borné (il y a une saturation maximale impossible à dépasser), tout comme la vitesse de la lumière est une limite en relativité (géométrie de Lorentz/Minkowski)27.